...
export default function CaseStudyDetailsClient({
  ...
  const shareOnTwitter = () => {
    ...
    const text = encodeURIComponent(
      `${caseStudy.title}${caseStudy.excerpt ? " - " + caseStudy.excerpt : ""}`,
    );
    window.open(
    ...
    if (navigator.share) {
      navigator
        .share({
          ...
          text: caseStudy.excerpt || caseStudy.title,
          url,
        })
        .catch(() => {
          if (navigator.clipboard) {
            navigator.clipboard
              ...
              .then(() => {
                alert(
                  "URL copied to clipboard! You can now paste it on Instagram.",
                );
              })
              .catch(() => {
                ...
                {relatedCaseStudies.length === 0 ? (
                  ...
                  relatedCaseStudies.map((related) => (
                    <div key={related.id} className="case-one-waber" style={{ marginBottom: '30px' }}>
                      <div>
                        <Link href={`/Casestudy/${(related.slug && related.slug.trim()) ? related.slug.trim() : related.id}`} className="case-bage">
                          Case Study
                        </Link>
                      </div>
                      ...
                      <div className="ser-btn m-0">
                       <Link
  href={`/Casestudy/${related.slug || related.id}`}
